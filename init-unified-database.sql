...
-- 2-1. 스터디 그룹 테이블
CREATE TABLE IF NOT EXISTS study.study_groups (
  id VARCHAR(36) PRIMARY KEY,
...
-- 2-2. 그룹 멤버 테이블
CREATE TABLE IF NOT EXISTS study.group_members (
  id VARCHAR(36) PRIMARY KEY,
...
-- 2-3. 모임 일정 테이블
CREATE TABLE IF NOT EXISTS study.meetings (
  id VARCHAR(36) PRIMARY KEY,
  group_id VARCHAR(36) NOT NULL REFERENCES study.study_groups(id) ON DELETE CASCADE,
  title VARCHAR(200) NOT NULL,
  description TEXT,
...
-- 2-4. 출석 관리 테이블
CREATE TABLE IF NOT EXISTS study.meeting_attendees (
  id VARCHAR(36) PRIMARY KEY,
...
-- 게시판 인덱스
CREATE INDEX IF NOT EXISTS idx_board_posts_category_id ON board.posts(category_id);
CREATE INDEX IF NOT EXISTS idx_board_posts_created_at ON board.posts(created_at);
CREATE INDEX IF NOT EXISTS idx_board_posts_is_hot ON board.posts(is_hot);
CREATE INDEX IF NOT EXISTS idx_board_posts_like_count ON board.posts(like_count);
CREATE INDEX IF NOT EXISTS idx_board_comments_post_id ON board.comments(post_id);
CREATE INDEX IF NOT EXISTS idx_board_comments_created_at ON board.comments(created_at);
CREATE INDEX IF NOT EXISTS idx_board_likes_post_id ON board.likes(post_id);
CREATE INDEX IF NOT EXISTS idx_board_likes_user_id ON board.likes(user_id);

-- 스터디 그룹 인덱스
CREATE INDEX IF NOT EXISTS idx_study_groups_category ON study.study_groups(category);
CREATE INDEX IF NOT EXISTS idx_study_groups_is_active ON study.study_groups(is_active);
CREATE INDEX IF NOT EXISTS idx_study_groups_created_at ON study.study_groups(created_at);
CREATE INDEX IF NOT EXISTS idx_group_members_group_id ON study.group_members(group_id);
CREATE INDEX IF NOT EXISTS idx_group_members_user_id ON study.group_members(user_id);
CREATE INDEX IF NOT EXISTS idx_meetings_group_id ON study.meetings(group_id);
CREATE INDEX IF NOT EXISTS idx_meetings_date ON study.meetings(date);
CREATE INDEX IF NOT EXISTS idx_meeting_attendees_meeting_id ON study.meeting_attendees(meeting_id);
CREATE INDEX IF NOT EXISTS idx_meeting_attendees_user_id ON study.meeting_attendees(user_id);
...
-- 게시판 트리거
DROP TRIGGER IF EXISTS update_board_posts_updated_at ON board.posts;
CREATE TRIGGER update_board_posts_updated_at 
    BEFORE UPDATE ON board.posts
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_board_comments_updated_at ON board.comments;
CREATE TRIGGER update_board_comments_updated_at 
    BEFORE UPDATE ON board.comments
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- 스터디 그룹 트리거
DROP TRIGGER IF EXISTS update_study_groups_updated_at ON study.study_groups;
CREATE TRIGGER update_study_groups_updated_at 
    BEFORE UPDATE ON study.study_groups
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_meetings_updated_at ON study.meetings;
CREATE TRIGGER update_meetings_updated_at 
    BEFORE UPDATE ON study.meetings
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- 알림 트리거
DROP TRIGGER IF EXISTS update_notifications_updated_at ON notification.notifications;
...
DO $$
BEGIN
    RAISE NOTICE '=== HIPPO 통합 데이터베이스 초기화 완료 ===';
    RAISE NOTICE '게시판 스키마: board';
    RAISE NOTICE '스터디 그룹 스키마: study';
    RAISE NOTICE '알림 스키마: notification';
    RAISE NOTICE '사용자: hippo_user';
    RAISE NOTICE '데이터베이스: hippo_unified_db';
    RAISE NOTICE '========================================';
END $$;
